    ...
    for v in root.children:
        if type(v.value) != str and  v.value(entry.attr[root.value], MEDIAN_MAP[root.value]):
           return Decision(v.nextNode, entry)
            ...
            return Decision(v.nextNode, entry)
    print('no terminal found in children')
    return 0 # only occurs if issue with tree construction
...
def Use_Numeric_Median(trainFile, testFile, replace=False):
        ...
        for line in f:
            ...
            for i in range(len(term)-1): 
                try:
                    ...
                    if replace and term[i] == '?':
                        unknowns.add(i)
                    if i in Attr.keys():
                        Attr[i].add(term[i])
                    else:
                        Attr[i] = {term[i]}
            val = 1 if int(term[-1]) > 0 else -1
            Labels.add(val) 
            S.append(entry(term[:-1], val)) 
    ...
    if replace:
        for i in unknowns:
            counts = {v:0 for v in Attr[i] if v != '?'}
            for s in S:
                if s.attr[i] != '?':
                    counts[s.attr[i]] += 1
            ...
            for line in f:
                ...
                if replace:
                    for i in range(len(term)):
                        if term[i] == '?':
                            term[i] = unkn_record[i-1]
                for i in numerics:
                    term[i+1] = int(term[i+1])
                tests.append(entry(term[1:], term[0]))
    return S, Attr, Labels, tests


def WrtieCSV(results):
    with open('resultsRF.csv', 'w+') as f:
        f.write("ID,Prediction\n")
        for r in results:
            f.write('{},{}\n'.format(r[0],r[1]))

#--------TESTING EXAMPLES---------
def TestForest(H, test_set):
    record = []
    for t in test_set:
        hypothesis = 0
        for h in H:
            hypothesis += Decision(h,t)
        l = 1 if hypothesis >= 0 else 0
        record.append((t.label, l))
    WrtieCSV(record)
